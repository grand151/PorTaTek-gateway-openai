...
class UserManager {
  ...
  setAdminStatus(userIdOrLogin, isAdmin) {
    // Support both userId format (github_123) and login string (admin-user)
    let userId = userIdOrLogin;
    if (!this.users[userId]) {
      // Try looking up by login if direct ID lookup fails
      const user = this.getUserByLogin(userIdOrLogin);
      if (user) {
        userId = user.id;
      } else {
        return null;
  ...
  updateUserPermissions(userIdOrLogin, permissions) {
    // Support both userId format (github_123) and login string (admin-user)
    let userId = userIdOrLogin;
    if (!this.users[userId]) {
      // Try looking up by login if direct ID lookup fails
